...
| MQTT            | mqtt.js client                         | EMQX Cloud TLS:8883 |
...
│
├── config/                         # [LEGACY] placeholder (runtime tidak digunakan)
├── models/                         # [LEGACY] placeholder (runtime tidak digunakan)
├── legacy/
│   └── sequelize-migrations/       # Arsip migration JS historis Sequelize
│       ├── 20251210120000-create-user-roles.js
│       ├── 20260318084551-add_prediction_tracking.js
│       └── 20260318085207-add_dehumidifier_column.js
├── migrations/                     # SQL migrations aktif
...
└── src/                            # ══════ SOURCE UTAMA ══════
   ...
   │   ├── models/
   │   │   └── index.ts            # Barrel export Drizzle db + schema
    ...
    │
    ├── mqtt/
   │   ├── client.ts               # MQTT lifecycle orchestrator
   │   ├── connection.ts           # MQTT connection factory + constants
   │   ├── messageRouter.ts        # MQTT topic/payload routing handlers
   │   ├── dedupStore.ts           # QoS1 deduplication store
   │   ├── payloadUtils.ts         # Payload parsing helpers
   │   └── logger.ts               # Centralized MQTT logger
    │
...
router.post(
  '/command',
  authMiddleware,
  roleBasedAuth(['admin', 'super_admin']),
  handler
);
```
...
| ------ | ------------- | ------------------------------------------------- |
| GET    | `/`           | `{ message, timestamp, environment, port }`       |
| GET    | `/health`     | `{ status: 'healthy', timestamp, uptime }`        |
| GET    | `/keep-alive` | `{ status: 'alive', message, timestamp, uptime }` |
| HEAD   | `/keep-alive` | `200 OK (empty body)`                             |
...
### 9.3 Alur Pemrosesan Pesan (`src/mqtt/messageRouter.ts`)
...
Modifikasi di `src/mqtt/messageRouter.ts` (dipanggil oleh orchestrator `src/mqtt/client.ts`):
...
server.ts (Entry Point)
  ...
  │
   ├── mqtt/client.ts (lifecycle orchestrator)
   │     └── mqtt/messageRouter.ts (hub pemrosesan data IoT)
   │           ├── → intrusiService.ingestIntrusiEvent()
