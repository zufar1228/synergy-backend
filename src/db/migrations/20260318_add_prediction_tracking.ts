...
module.exports = {
  async up(queryInterface: any, Sequelize: any) {
    ...
    if (!tableDescription.last_prediction_temperature) {
      addColumns.push(
        queryInterface.addColumn('devices', 'last_prediction_temperature', {
          type: Sequelize.REAL,
          allowNull: true,
          defaultValue: null
        })
    ...
    if (!tableDescription.last_prediction_humidity) {
      addColumns.push(
        queryInterface.addColumn('devices', 'last_prediction_humidity', {
          type: Sequelize.REAL,
          allowNull: true,
          defaultValue: null
        })
    ...
    if (!tableDescription.last_prediction_co2) {
      addColumns.push(
        queryInterface.addColumn('devices', 'last_prediction_co2', {
          type: Sequelize.REAL,
          allowNull: true,
          defaultValue: null
        })
    ...
    if (!tableDescription.actuator_fan_on_reason) {
      addColumns.push(
        queryInterface.addColumn('devices', 'actuator_fan_on_reason', {
          type: Sequelize.TEXT,
          allowNull: true,
          defaultValue: null
        })
    ...
    if (!tableDescription.actuator_dehumidifier_on_reason) {
      addColumns.push(
        queryInterface.addColumn('devices', 'actuator_dehumidifier_on_reason', {
          type: Sequelize.TEXT,
          allowNull: true,
          defaultValue: null
        })
  ...
  async down(queryInterface: any) {
    ...
    await Promise.all(
      columnsToRemove.map((col) =>
        queryInterface.removeColumn('devices', col).catch((err: any) => {
