...
const log = {
  debug: (...args: any[]) => currentLevel <= 0 && console.log('[MQTT]', ...args),
  info: (...args: any[]) => currentLevel <= 1 && console.log('[MQTT]', ...args),
  warn: (...args: any[]) => currentLevel <= 2 && console.warn('[MQTT]', ...args),
  error: (...args: any[]) => currentLevel <= 3 && console.error('[MQTT]', ...args),
};
      ...
      try {
        ...
        if (topicParts.length === 7 && topicParts[6] === 'status') {
          ...
          try {
            ...
            if (
              ...
              if (statusData.state) extraFields.intrusi_system_state = statusData.state;
              ...
              if (statusData.vbat_v !== undefined) extraFields.vbat_voltage = parseFloat(statusData.vbat_v);
              if (statusData.vbat_pct !== undefined) extraFields.vbat_pct = parseInt(statusData.vbat_pct, 10);
        ...
        if (topicParts.length === 8 && topicParts[6] === 'sensors') {
          ...
          if (systemType === 'intrusi') {
            ...
            } else if (data.type === 'FORCED_ENTRY_ALARM' || data.type === 'UNAUTHORIZED_OPEN') {
