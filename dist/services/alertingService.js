...
const processSensorDataForAlerts = async (deviceId, systemType, data) => {
    ...
    const device = (await models_1.Device.findByPk(deviceId, {
        include: [{ model: models_1.Area, as: 'area', include: [{ model: models_1.Warehouse, as: 'warehouse' }] }]
    }));
    ...
    const timestamp = (0, date_fns_1.format)(new Date(), "dd MMMM yyyy, HH:mm:ss 'WIB'", { locale: locale_1.id });
