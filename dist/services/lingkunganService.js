...
exports.updateLingkunganLogStatus =
  exports.getLingkunganStatus =
  exports.getChartData =
  exports.getLingkunganSummary =
  exports.getLingkunganLogs =
  exports.switchToAutoMode =
  exports.handleManualControl =
  exports.sendActuatorCommand =
  exports.handlePredictionResult =
  exports.ingestSensorData =
    void 0;
 ...
 */
const triggerPrediction = async (deviceId) => {
  try {
    // Get the last 60 readings for the LSTM sequence (1 hour of 1‑min data)
    ...
    client_1.client.publish(
      ML_PREDICT_REQUEST_TOPIC,
      payload,
      { qos: 1 },
      (err) => {
        if (err) {
...
const handleFirmwareSafetyCheck = async (data) => {
  ...
  if (
    ...
    if (device.fan_state === 'ON' || device.dehumidifier_state === 'ON') {
      console.log(
      ...
      await models_1.Device.update(
        { fan_state: 'OFF', dehumidifier_state: 'OFF' },
        { where: { id: data.device_id } }
      );
    }
  }
 ...
 */
const sendActuatorCommand = async (deviceId, command) => {
  const device = await models_1.Device.findByPk(deviceId, {
    include: [
...
const getLingkunganStatus = async (device_id) => {
  ...
  if (!device) throw new apiError_1.default(404, 'Perangkat tidak ditemukan.');
  const latest = await models_1.LingkunganLog.findOne({
    where: { device_id },
    order: [['timestamp', 'DESC']]
  });
