...
const findAndNotifyRepeatDetections = async () => {
    ...
    const newDetections = await keamananLog_1.default.findAll({
        where: {
            detected: true,
    ...
    for (const [identityKey, detections] of detectionMap.entries()) {
        ...
        const recentNotifiedCount = await keamananLog_1.default.count({
            where: {
        ...
        if (recentNotifiedCount > 0) {
            ...
            await keamananLog_1.default.update({ notification_sent_at: new Date() }, { where: { id: { [sequelize_1.Op.in]: detections.map((d) => d.id) } } });
