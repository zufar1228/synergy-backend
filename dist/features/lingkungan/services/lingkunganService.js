...
const handleFirmwareSafetyCheck = async (data, _device) => {
    ...
    if (tempSafe && freshDevice.fan_state === 'ON') {
        ...
        await (0, exports.sendActuatorCommand)(data.device_id, { fan: 'OFF' }, freshDevice);
        await drizzle_1.db
            .update(schema_1.devices)
            .set({ fan_state: 'OFF' })
            .where((0, drizzle_orm_1.eq)(schema_1.devices.id, data.device_id));
    ...
    if (humSafe && co2Safe && freshDevice.dehumidifier_state === 'ON') {
        ...
        await (0, exports.sendActuatorCommand)(data.device_id, { dehumidifier: 'OFF' }, freshDevice);
        await drizzle_1.db
            .update(schema_1.devices)
    ...
    if (allSafe && !recoveryNotifiedSet.has(data.device_id) &&
        ...
        await lingkunganAlertingService.processLingkunganAlert(data.device_id, ['Kondisi lingkungan kembali stabil. Semua aktuator telah dinonaktifkan.'], data, 'RECOVERY');
    }
};
/**
...
const handleActualThresholdControl = async (data, _device) => {
    ...
    if (!needFan && !needDehum) {
        if (actualAlertCountMap.has(data.device_id)) {
            console.log(`[LingkunganService] Actuator(s) already active for ${data.device_id}. Notifications stopped.`);
            actualAlertCountMap.delete(data.device_id);
        }
        return;
    }
    ...
    console.log(`[LingkunganService] Actual threshold alert ${currentCount}/${ACTUAL_ALERT_MAX} for device ${data.device_id}`);
    // Build alert messages
    const alerts = [];
    if (needFan) {
        alerts.push(`Suhu saat ini ${data.temperature.toFixed(1)}°C (>= ${FAILSAFE_TEMP_THRESHOLD}°C). Kipas belum aktif.`);
    }
    if (needDehum && data.humidity >= FAILSAFE_HUMIDITY_THRESHOLD) {
        alerts.push(`Kelembapan saat ini ${data.humidity.toFixed(1)}% (>= ${FAILSAFE_HUMIDITY_THRESHOLD}%). Dehumidifier belum aktif.`);
    }
    if (needDehum && data.co2 >= FAILSAFE_CO2_THRESHOLD) {
        alerts.push(`CO2 saat ini ${data.co2.toFixed(0)}ppm (>= ${FAILSAFE_CO2_THRESHOLD}ppm). Dehumidifier belum aktif.`);
    }
    // Send notification (up to 3 times)
    if (currentCount <= ACTUAL_ALERT_MAX) {
        alerts.push(`⚠️ Notifikasi ke-${currentCount}/${ACTUAL_ALERT_MAX}. Silakan aktifkan aktuator melalui Mode Manual di dashboard.`);
    ...
    if (currentCount >= ACTUAL_ALERT_MAX) {
        console.log(`[LingkunganService] ${ACTUAL_ALERT_MAX} notifications sent. Auto-activating actuators for ${data.device_id}.`);
        const command = {};
        const updateData = {};
        if (needFan) {
            command.fan = 'ON';
